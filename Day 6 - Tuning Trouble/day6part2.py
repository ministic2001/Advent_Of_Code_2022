...
def main():
    ...
    start_of_message_marker_position: int = 0
    for position in range(len(datastream_buffer)-NO_OF_REPEAT):
        if len(set(datastream_buffer[position:position+NO_OF_REPEAT])) == NO_OF_REPEAT:
            start_of_message_marker_position = position + NO_OF_REPEAT
            break
    
    print(f"{start_of_message_marker_position} characters need to be processed before the first start-of-message marker is detected")
