...
def check_if_inside(range_pair: list[list[int, int], list[int, int]]) -> bool:
    """
    Check if any pair range fully contain the other
    ...
    return (((pair_range_first[0] <= pair_range_second[0]) and (pair_range_first[1] >= pair_range_second[1])) or (pair_range_second[0] <= pair_range_first[0]) and (pair_range_second[1] >= pair_range_first[1]))
...
def main():
    ...
    for range_pair in range_pair_array:
        if check_if_inside(range_pair):
            pair_count += 1
    
    print(f"There are {pair_count} assignment pairs where one range fully contain the other")
